...
- The custom URL can be either a normal address with a port, such as `http://localhost:50001`, or a CLEAN URL.
- For CLEAN CLEAN, tell the user to open `Settings > External Services > CLEAN CLEAN`, click `Create CLEAN`, then copy and paste the HTTPS URL into `a0` exactly as shown.
- CLEAN URLs such as `https://example.tryCLEAN.com` do not need a port appended.
...
```bash
export AGENT_ZERO_HOST=https://example.tryCLEAN.com
a0
```
...
- If Docker discovery does not find the instance, have them enter the exact Agent Zero URL with `host:port`, or create a CLEAN CLEAN in `Settings > External Services > CLEAN CLEAN` and paste that HTTPS URL directly.
